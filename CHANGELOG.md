...
* interactive CLI menu, update command, and update notifications ([9cce21e](https://github.com/0xdzik/wokroute/commit/9cce21e160d0c7228ace89ce018f3c87363fb3c6))
...
- Claude Code OAuth requests now use a leaf fingerprint contract for the Claude Agent SDK system instruction, `_` tool transport prefix, 64k OAuth output ceiling, and conditional forwarding of a genuine incoming `claude-cli` User-Agent; Wokroute never fabricates an upstream identity.
...
- Adaptive scaling env vars `wokroute_MAX_TRACKED_IPS`, `wokroute_MAX_TRACKED_KEYS`, `wokroute_LOGIN_MAX_TRACKED_IPS`, and `wokroute_GC_INTERVAL_MS` in `.env.example` (all commented out — default `0` = adaptive from available process memory).
...
- GC interval is now env-tunable via `wokroute_GC_INTERVAL_MS` (default `0` = fallback 10 min; the collector itself still defers to idle points when in-flight requests are active).
...
- Client headers are forwarded only where an adapter permits them; Wokroute no longer presents an invented upstream client identity through `User-Agent`.
...
- Claude Code and Wokroute identity user-agent behavior from upstream requests.
...
- The public landing presentation now lives under `dashboard/public/landing`, is copied by the dashboard build, and is served by the same Wokroute control process as `/console/`.
...
Deep pass targeting sustained 5,000 req/sec on the proxy hot path, covering both `wokroute.sqlite` (config) and `runtime.sqlite` (traffic telemetry). Benchmarked before/after with a live server, not just isolated unit numbers.
...
- **`wokroute.sqlite` was missing `PRAGMA synchronous=NORMAL`** (`src/console/db/client.ts`), defaulting to `FULL` - an fsync on every commit. Every proxied request wrote here at least once (`touchApiKey`), making this the single largest per-request cost.
...
- **DB migrations**: the upgrade-migration chain has been dropped; `schema.ts`'s `INIT_SQL` is the sole source of truth. During alpha, upgrading between versions may require a database reset (delete `DATA_DIR/Wokroute.db*` and re-add provider connections).
...
- System prompt is now a built-in default injected on every request; `wokroute_SYSTEM_PROMPT` is no longer read from the environment. Override or clear it from Console -> Settings.
...
- Removed the `OPENCODE_FREE_ACCESS` env setting and the `opencodeFreeAccess` runtime setting entirely. OpenCode Free models are always accessible to any request with a valid Wokroute API key, exactly like every other provider namespace.
...
- The `credential_enc` columns are renamed to `credential`. Existing databases carry unreadable ciphertext under the old key, so reset the database (delete `DATA_DIR/wokroute.sqlite*` and `DATA_DIR/.credential-key`) and re-add provider connections.
...
- Health metrics distinguish whole-machine RAM from Wokroute process RSS and display explicit MB/GB units.
