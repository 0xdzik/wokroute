...
async function streamModelStudioChat(
  ...
  if (!res.ok || !res.body) {
    ...
    try { const errBody = (await res.json()) as { error?: { message?: string } }; if (errBody.error?.message) message = errBody.error.message; } catch { /* keep generic */ }
  ...
  for (;;) {
    ...
    for (const frame of frames) {
      ...
      if (choiceDelta?.content || choiceDelta?.reasoning_content) {
        if (!firstTokenRecorded) { firstTokenRecorded = true; delta.onFirstToken(performance.now() - startTime); }
      }
